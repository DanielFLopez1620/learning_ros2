...
private:
    ...
    void on_timer()
    {
        // Obtain and assign names of the transform of interest, note the change
        // of the follower name, in order to generate a runtime error to check
        // when debugging tf2.
        std::string fromFrameRel = target_frame_.c_str();
        std::string toFrameRel = "NotFollower";
            ...
            if(turtle_spawned_)
            {
                ...
                try
                {
                    ...
                    t_stamp = tf_buf_->lookupTransform(
                        toFrameRel, fromFrameRel, this->now());
                }
